...
$(REQUIREMENTS_FILE): setup.py
	...
	/bin/bash -c "source $(VENV_PATH)/bin/activate && python setup.py develop && pip freeze -l | grep -v putsync > $(REQUIREMENTS_FILE) && pip wheel -r $(REQUIREMENTS_FILE) -w $(WHEEL_PATH)"
