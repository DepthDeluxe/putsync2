...
$(REQUIREMENTS_FILE): setup.py
	...
	/bin/bash -c "source $(VENV_PATH)/bin/activate && pip install pex wheel && python setup.py develop && pip freeze -l | grep -v pkg-resources==0.0.0 | grep -v putsync > $(REQUIREMENTS_FILE) && pip wheel -r $(REQUIREMENTS_FILE) -w $(WHEEL_PATH)"
